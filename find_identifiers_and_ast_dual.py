...
from find_identifiers_and_ast import build_report_for_identifiers
...
def write_combined_payload_file(report: dict, out_path: Path, target_ids: List[str]):
    ...
    # 코드: 각 파일의 전체 내용을 순서대로 연결. 파일 경계는 주석으로 명시.
...
def write_per_identifier_payload_files(report: dict, out_dir: Path, target_ids: List[str], ctx_lines: int):
    """
...
def main():
    ...
    write_per_identifier_payload_files(report, Path(args.per_id_dir), ids, args.ctx_lines)
