...
def main():
    ...
    print("[Swingft_String_Encryption] Swift 도구로 문자열 분석 시작...")
    cmd_a = ["swift", "run", "Swingft_Encryption", str(root), str(cfg)]

    print("[Swingft_String_Encryption] 빌드 타겟 분석 시작...")
    ...
    print("[Swingft_String_Encryption] 병렬 실행 중... (시간이 오래 걸릴 수 있습니다)")
    rcA, rcB = run_parallel(cmd_a, "A", cmd_b, "B", cwd=cfg_dir)
    if rcA != 0:
        print(f"[Swingft_String_Encryption] ERROR: Swift 도구 실행 실패 (code {rcA})")
        sys.exit(3)
    if rcB != 0:
        print(f"[Swingft_String_Encryption] ERROR: 빌드 타겟 분석 실패 (code {rcB})")
        sys.exit(3)
    
    print("[Swingft_String_Encryption] 문자열 분석 및 빌드 타겟 분석 완료!")
