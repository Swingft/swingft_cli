...
def copy_StringSecurity_folder(source_root):
    print(f"[DEBUG] copy_StringSecurity_folder 호출됨: {source_root}")
    local_path = os.path.join(os.path.dirname(__file__), "StringSecurity")
    print(f"[DEBUG] StringSecurity 로컬 경로: {local_path}")
    if not os.path.exists(local_path):
        print(f"[DEBUG] StringSecurity 폴더가 존재하지 않음: {local_path}")
        return
    print(f"[DEBUG] StringSecurity 폴더 존재 확인됨")
    for dirpath, dirnames, _ in os.walk(source_root):
        print(f"[DEBUG] 디렉토리 탐색 중: {dirpath}, dirnames: {dirnames}")
        for d in dirnames:
            if d.endswith(('.xcodeproj', '.xcworkspace')):
                target = os.path.join(dirpath, "StringSecurity")
                print(f"[DEBUG] 프로젝트 파일 발견: {d}, 타겟: {target}")
                if not os.path.exists(target):
                    print(f"[DEBUG] StringSecurity 폴더 복사 시작: {local_path} -> {target}")
                    shutil.copytree(local_path, target)
                    print(f"[DEBUG] StringSecurity 폴더 복사 완료")
                else:
                    print(f"[DEBUG] StringSecurity 폴더가 이미 존재함: {target}")
                return
    print(f"[DEBUG] 프로젝트 파일을 찾지 못함")
